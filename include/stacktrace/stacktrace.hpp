...
#include <stacktrace/boost-modified/container_hash/hash_fwd.hpp>
...
class basic_stacktrace {
    std::vector<stacktrace_::frame, Allocator> impl_;
    ...
    STACKTRACE_NOINLINE void init(std::size_t frames_to_skip, std::size_t max_depth) {
        ...
        try {
            {   // Fast path without additional allocations
                ...
                const std::size_t frames_count = stacktrace_::detail::this_thread_frames::collect(buffer, buffer_size < max_depth ? buffer_size : max_depth, frames_to_skip + 1);
            ...
            do {
                const std::size_t frames_count = stacktrace_::detail::this_thread_frames::collect(&buf[0], buf.size() < max_depth ? buf.size() : max_depth, frames_to_skip + 1);
...
public:
    typedef typename std::vector<stacktrace_::frame, Allocator>::value_type             value_type;
    typedef typename std::vector<stacktrace_::frame, Allocator>::allocator_type         allocator_type;
    typedef typename std::vector<stacktrace_::frame, Allocator>::const_pointer          pointer;
    typedef typename std::vector<stacktrace_::frame, Allocator>::const_pointer          const_pointer;
    typedef typename std::vector<stacktrace_::frame, Allocator>::const_reference        reference;
    typedef typename std::vector<stacktrace_::frame, Allocator>::const_reference        const_reference;
    typedef typename std::vector<stacktrace_::frame, Allocator>::size_type              size_type;
    typedef typename std::vector<stacktrace_::frame, Allocator>::difference_type        difference_type;
    typedef typename std::vector<stacktrace_::frame, Allocator>::const_iterator         iterator;
    typedef typename std::vector<stacktrace_::frame, Allocator>::const_iterator         const_iterator;
    typedef typename std::vector<stacktrace_::frame, Allocator>::const_reverse_iterator reverse_iterator;
    typedef typename std::vector<stacktrace_::frame, Allocator>::const_reverse_iterator const_reverse_iterator;
    ...
    STACKTRACE_FORCEINLINE basic_stacktrace() noexcept
        : impl_()
    {
    ...
    STACKTRACE_FORCEINLINE explicit basic_stacktrace(const allocator_type& a) noexcept
        : impl_(a)
    {
        init(0 , static_cast<std::size_t>(-1));
    }
    ...
    STACKTRACE_FORCEINLINE basic_stacktrace(std::size_t skip, std::size_t max_depth, const allocator_type& a = allocator_type()) noexcept
        : impl_(a)
    {
        init(skip , max_depth);
    }
    ...
    basic_stacktrace& operator=(basic_stacktrace&& st)
        noexcept(( std::is_nothrow_move_assignable< std::vector<stacktrace_::frame, Allocator> >::value ))
    {
        impl_ = std::move(st.impl_);
        return *this;
    }
    ...
    ///
    /// @b Async-Handler-Safety: Safe.
    STACKTRACE_FORCEINLINE explicit operator bool() const noexcept
    {
    ...
    const std::vector<stacktrace_::frame, Allocator>& as_vector() const noexcept {
        return impl_;
    }
    ...
    ///
    /// @param begin Begining of the memory where the stacktrace was saved using the stacktrace_::safe_dump_to
    ///
    /// @param buffer_size_in_bytes Size of the memory. Usually the same value that was passed to the stacktrace_::safe_dump_to
    ///
...
std::string to_string(const basic_stacktrace<Allocator>& bt) {
    ...
    return stacktrace_::detail::to_string(&bt.as_vector()[0], bt.size());
}
...
std::basic_ostream<CharT, TraitsT>& operator<<(std::basic_ostream<CharT, TraitsT>& os, const basic_stacktrace<Allocator>& bt) {
    return os << stacktrace_::to_string(bt);
}

/// This is the typedef to use unless you'd like to provide a specific allocator to stacktrace_::basic_stacktrace.
